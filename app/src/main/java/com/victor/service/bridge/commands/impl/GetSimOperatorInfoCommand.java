package com.victor.service.bridge.commands.impl;

import android.app.Activity;
import android.webkit.WebView;

import androidx.annotation.Nullable;

import com.victor.service.bridge.commands.DeviceCommand;
import com.victor.service.bridge.commands.base.Command;
import com.victor.service.provider.SimCardProvider;

public class GetSimOperatorInfoCommand extends Command {

    private SimCardProvider simCardProvider = new SimCardProvider();

    @Override
    public DeviceCommand getCommand() {
        return DeviceCommand.getSimOperatorInfo;
    }

    @Override
    protected Object execute(String commandId, @Nullable String jsonParams, Activity activity, WebView webView) {
        return simCardProvider.getSimOperatorInfo(activity);
    }

}
